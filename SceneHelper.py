...
def stop_record(status):
    ...
    if start_time:
        ws.call(requests.StopRecord())
        ws_excel.append([start_time, status])
        wb.save(f"{init_time}-{scene}.xlsx")
        print(f"\n{start_time} 录制已停止，状态为：{status}。\n")
        status_label.config(text=f"录制已停止，状态为：{status}。")
        start_btn.config(image=start_recording_img)  # 恢复为开始录制的图像
        start_time = None
